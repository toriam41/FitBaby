...
const ListItem = ({item, theme, routineState, navigation}) => {
  ...
  const addExercise = () => {
    if (!routineList.includes(item)) {
      ...
      console.log('added');
      const exercisesRef = addDoc(
        collection(
          db,
          'users',
          'P6MVzw12GcXSYfRt4t2kwm73etl1',
          'routines',
          'routine',
          'exercises',
        ),
      ...
      // updateDoc(doc(db, 'exercises', exercisesRef.id), {
      //   id: item.id,
      // });
    }
  };
  ...
  return (
    <>
      <Pressable
        style={[styles.item, styles.container, {backgroundColor: theme}]}
        onPress={handlePress}>
  ...
  const fetchExerciseCards = category => {
    ...
    async function fec() {
      ...
      const options = {
        ...
        headers: {
          'X-RapidAPI-Key':
            '4b348e1878mshc15d211c5f760aap1b7789jsn325a310c1c8c',
          ...
          renderItem={({item}) => {
            ...
            return (
              <ListItem
                item={item}
                theme={section.theme}
                routineState={[routineList, setRoutineList]}
